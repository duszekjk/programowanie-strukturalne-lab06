...
if (-not (Test-Path $BuildBat)) {
    throw "Unreal Build.bat not found: $BuildBat. Pass -EngineRoot if UE is installed elsewhere."
}
...
$Command += " && `"$BuildBat`" IpcCharacterWorldEditor Win64 $Configuration -Project=`"$ProjectFile`" -WaitMutex"

Write-Host "Building Unreal editor module..."
cmd.exe /d /c $Command
